...
class EmployeeProfilePage extends StatefulWidget {
  ...
  const EmployeeProfilePage(
      {super.key, required this.authToken, required this.empId});
...
class _EmployeeProfilePageState extends State<EmployeeProfilePage> {
  ...
  Future<void> fetchEmployee() async {
    try {
      ...
      if (data["status"] == "success") {
        setState(() {
          employee = EmployeeDetails.fromJson(data["employee_details"]);
          isLoading = false;
        });
      }
    } catch (e) {
          ...
          Expanded(
            ...
            child: Text(title,
                style: TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: context.fontSize(
                        mobile: 14, tablet: 15, desktop: 15))),
          ),
          Expanded(
            flex: 5,
            child: Text(value,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(
        // ✅ MaxWidthContainer from responsive_layout.dart
        child: MaxWidthContainer(
          child: SingleChildScrollView(
            // ✅ context.pagePadding from responsive_layout.dart
            padding: context.pagePadding,
            child: Column(
              children: [
                SizedBox(height: context.rv(
                    mobile: 20.0, tablet: 30.0, desktop: 30.0)),
                CircleAvatar(
                  radius: context.rv(
                      mobile: 50.0, tablet: 65.0, desktop: 70.0),
                ...
                SizedBox(height: context.rv(
                    mobile: 30.0, tablet: 36.0, desktop: 40.0)),
